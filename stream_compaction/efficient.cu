...
namespace StreamCompaction {
    namespace Efficient {
        ...
        {          
            ...
            dev_kernWriteToSharedMemory(n, temp, idata);

            //If n is not a power of 2, get closest power of 2 and fill the rest of the array with zeroes.
            if (((1 << ilog2ceil(n)) - n) > 0)
            {
                POT = 1 << ilog2ceil(n);
                dev_kernFillExtraZeros(POT, n, temp);
                N = POT;
            }
            else
            {
            ...
            {
                __syncthreads();
                dev_kernUpsweep(d, offset, N, temp);
                offset *= 2;
            }

            //Clear Last Element 
            dev_kernClearLastElem(N, temp);
            ...
            for (int d = 1; d < N; d *= 2)
            {
                offset >>= 1;
                __syncthreads();
                dev_kernDownsweep(d, offset, N, temp);
            }
            __syncthreads();

            //Write back values to host from device 
            dev_kernWriteFromSharedMemory(n, temp, odata);
        ...
        int compactSharedMemory(int n, int* odata, const int* idata)
        {
            ...
            kernScanHelperSharedMemory << <fullBlocksPerGrid, blockSize >> > (n, dev_scanResult, dev_tempArray); 
            checkCUDAErrorFn("kernScanHelperSharedMemory failed!");
            ...
            cudaMemcpy(scanRes, dev_scanResult, n * sizeof(int), cudaMemcpyDeviceToHost);
            checkCUDAErrorFn("cudaMemCpy dev_scanres to scanres failed!");
            ...
            if (idata[n - 1] != 0)
            {
                count = dev_scanResult[n - 1] + 1;
            }
            else
            {
                count = dev_scanResult[n - 1];
            }
